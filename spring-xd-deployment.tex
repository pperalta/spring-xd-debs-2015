...
While XD does not subscribe to a specific deployment model (Lambda, 
Reactive or other), it does allow for a user to architect the deployment type 
that meets their needs.  Note that this is not an exhaustive list of all 
deployment types, see section~\ref{sec:Use Cases} for descriptions of real 
world deployments.

\subsection{Lambda Architecture}

One of the architectures a user can implement using XD is the
Lambda Architecture, introduced by Nathan Marz \cite{lambda-architecture-paper}. 
Is a generic, scalable and fault tolerant data  processing architecture that 
